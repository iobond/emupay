...
export class IncomingDataProvider {
  ...
  private isValidAibUri(data: string): boolean {
    data = this.sanitizeUri(data);
    return !!this.bwcProvider.getAibcore().URI.isValid(data);
  }
  ...
  private isValidAibAddress(data: string): boolean {
      return !!(
          this.bwcProvider.getAibcore().Address.isValid(data, 'livenet') ||
          this.bwcProvider.getAibcore().Address.isValid(data, 'testnet')
      );
  }
  ...
  private handleAibUri(data: string, redirParams?: RedirParams): void {
      this.logger.debug('Incoming-data: AIB URI');
      let amountFromRedirParams =
          redirParams && redirParams.amount ? redirParams.amount : '';
      const coin = Coin.AIB;
      let parsed = this.bwcProvider.getAibcore().URI(data);
  ...
  private handlePlainAibAddress(
      data: string,
      redirParams?: RedirParams
  ): void {
      this.logger.debug('Incoming-data: AIB plain address');
      const coin = Coin.AIB;
  ...
  public redir(data: string, redirParams?: RedirParams): boolean {
    ...
    if (this.isValidPayProNonBackwardsCompatible(data)) {
      ...
      return true;
        ...
        // AIB URI
    } else if (this.isValidAibUri(data)) {
        this.handleAibUri(data, redirParams);
        return true;
      ...
      return true;
        ...
        // Plain Address (AIB)
    } else if (this.isValidAibAddress(data)) {
        this.handlePlainAibAddress(data, redirParams);
        return true;
    ...
    if (this.isValidPayProNonBackwardsCompatible(data)) {
      return {
        ...
        return {
            data,
            type: 'AibUri',
            title: this.translate.instant('AIB URI')
        };
      ...
      return {
        ...
        return {
            data,
            type: 'AibAddress',
            title: this.translate.instant('AIB Address')
        };
