...
export class ProfileProvider {
  ...
  constructor(
    ...
    /*if (this.shareService.walletTypes.length == 0) {
      this.shareService.initData();
    }*/
  }
  ...
  private updateWalletSettings(wallet): void {
    ...
    let defaultColor =
      this.appProvider.info.nameCase == 'Copay' ? '#1abb9b' : '#2963af';
    ...
    return new Promise((resolve, reject) => {
      ...
      walletClient.importFromMnemonic(
        words,
        {
          ...
          coin: opts.coin,
          walletTypes: this.shareService.walletTypes,
        },
        err => {
          if (err) {
  ...
  public createDefaultWallet(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      opts.walletTypes = this.shareService.walletTypes;

      opts.coin = this.shareService.defalutWalletType || Coin.AIB.toUpperCase();
  ...
  public getWallets(opts?) {
    ...
    if (opts.coin) {
      ret = _.filter(ret, x => {
        return x.credentials.coin == opts.coin.toUpperCase();
      });
    }
        ...
        updateNotifications(wallet)
          .then(() => {
            ...
            _.each(n, x => {
              ...
              var symbol = x.wallet.coin;
              var tokenInfo = this.shareService.getTokenInfo(symbol);
              if (tokenInfo) {
                if (!!tokenInfo.factor && tokenInfo.factor > 0) {
                  x.data.factor = tokenInfo.factor;
                }
              }
            });
