...
export class AddressProvider {
  ...
  constructor(private bwcProvider: BwcProvider) {
    ...
    this.aibcore = this.bwcProvider.getAibcore();
    this.Bitcore = {
      btc: {
        lib: this.bitcore,
        translateTo: 'bch'
  ...
  public getCoin(address: string) {
    address = address.replace(/^(bitcoincash:|bchtest:|bitcoin:|aib:)/i, '');
    try {
      ...
      try {
        ...
        try {
            new this.Bitcore['aib'].lib.Address(address);
            return 'aib';
        } catch (e) {
            return null;
        }
    ...
    try {
      ...
      try {
        network = this.bwcProvider.getBitcoreCash().Address(address).network
          ...
          try {
              network = this.bwcProvider.getAibcore().Address(address).network
                  .name;
          } catch (e) {
              // return null
          }
  ...
  public validateAddress(address: string) {
    ...
    let AddressAib = this.aibcore.Address;
    let isLivenet = Address.isValid(address, 'livenet');
    let isTestnet = Address.isValid(address, 'testnet');
    let isLivenetAib = AddressAib.isValid(address, 'livenet');
    let isTestnetAib = AddressAib.isValid(address, 'testnet');
    let isLivenetCash = AddressCash.isValid(address, 'livenet');
    let isTestnetCash = AddressCash.isValid(address, 'testnet');
    return {
      address,
      isValid: isLivenetAib || isTestnetAib || isLivenet || isTestnet || isLivenetCash || isTestnetCash,
      network: isTestnetAib || isTestnet || isTestnetCash ? 'testnet' : 'livenet',
  ...
  public checkCoinAndNetworkFromAddr(
    ...
    if (this.isValid(address)) {
      ...
      return ( addressData.coin.toUpperCase() == coin || walletTypes.includes(addressData.coin.toUpperCase()))&&
  ...
  public extractAddress(address: string): string {
    let extractedAddress = address
      .replace(/^(bitcoincash:|bchtest:|bitcoin:|aib:)/i, '')
      .replace(/\?.*/, '');
  ...
  public isValid(address: string): boolean {
    ...
    let AddressCash = this.bitcoreCash.Address;
    let URIAib = this.aibcore.URI;
    let AddressAib = this.aibcore.Address;

    // Bip21 uri
    let uri, isAddressValidLivenet, isAddressValidTestnet;
    if (/^aib:/.test(address)) {
        let isUriValid = URIAib.isValid(address);
        if (isUriValid) {
            uri = new URI(address);
            isAddressValidLivenet = AddressAib.isValid(
                uri.address.toString(),
                'livenet'
            );
            isAddressValidTestnet = AddressAib.isValid(
                uri.address.toString(),
                'testnet'
            );
        }
    ...
    let regularAddressAibLivenet = AddressAib.isValid(address, 'livenet');
    let regularAddressAibTestnet = AddressAib.isValid(address, 'testnet');
    ...
    if (
      regularAddressAibLivenet ||
      regularAddressAibTestnet ||
