...
export class WalletProvider {
  ...
  public getStatus(wallet, opts): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      let cacheBalance = (wallet, balance): void => {
        ...
        if (this.shareService && this.shareService.isValidToken(wallet.coin)) {
          if (!!cache.balance.tokens && cache.balance.tokens.length > 0) {
            var tokensBalance = cache.balance.tokens.filter(token => {
              return token.symbol == wallet.coin;
            });
            balance = tokensBalance[0];
            balance.availableAmount = balance.totalAmount;
            balance.totalConfirmedAmount = balance.totalConfirmedAmount;
          } else {
            balance.totalAmount = 0;
            balance.lockedAmount = 0;
            balance.availableAmount = 0;
            balance.lockedConfirmedAmount = 0;
            balance.availableConfirmedAmount = 0;
            balance.totalConfirmedAmount = 0;
          }
        }
        ...
        // STR
        /*if (opts.shareService && opts.shareService.isValidToken(wallet.coin)) {
            var tokensData = opts.shareService.tokens.filter(token => {
               return token.symbol == wallet.coin;
            });
            var tokenData = tokensData[0]; //  only 1 element should be retured TODO: add size check
            cache.totalBalanceStr = this.txFormatProvider.formatAmountStr(
                wallet.coin,
                cache.totalBalanceSat * tokenData.factor
            );
            cache.lockedBalanceStr = this.txFormatProvider.formatAmountStr(
                wallet.coin,
                cache.lockedBalanceSat * tokenData.factor
            );
            cache.availableBalanceStr = this.txFormatProvider.formatAmountStr(
                wallet.coin,
                cache.availableBalanceSat * tokenData.factor
            );
            cache.spendableBalanceStr = this.txFormatProvider.formatAmountStr(
                wallet.coin,
                cache.spendableAmount * tokenData.factor
            );
            cache.pendingBalanceStr = this.txFormatProvider.formatAmountStr(
                wallet.coin,
                cache.pendingAmount * tokenData.factor
            );
        } else {*/
        var amountUnit = wallet.coin;
        if (!cache.balance.tokens && cache.balance.totalBalanceSat > 0)
          amountUnit = BaseCoin;
  ...
  public getTokens(): string[] {
    ...
    return walletClient.getWalletTypes(opts);
  }

  public verifyToken(wallet, tx) {
    var tokenInfo = this.shareService.getTokenInfo(wallet.coin);

    return new Promise((resolve, reject) => {
      wallet.verifyToken({ 'tokenid': tokenInfo.txid, 'txid': tx.txid }, (err, result) => {
        if (err) return reject(err);
        return resolve(result);
      });
    });
