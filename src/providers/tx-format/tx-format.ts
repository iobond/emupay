...
import { BaseCoin } from "../../environments/prod";
...
export class TxFormatProvider {
  ...
  public processTx(wallet, tx, shareService: ShareService, walletProvider, isNewTx?: boolean) {
    ...
    if (tx.outputs && tx.outputs.length) {
      ...
      if (outputsNr > 1 && !!shareService && isNewTx === true)
        this.normlizeOutputAmount(wallet.coin, tx, shareService);
      ...
      if (tx.action != 'received') {
        ...
        tx.amount = _.reduce(
          tx.outputs,
          (total, o) => {
            o.amountStr = this.formatAmountStr(wallet.coin, o.amount);
            o.alternativeAmountStr = this.formatAlternativeStr(wallet.coin, o.amount);
      ...
      if (tx.toAddress && wallet.coin == 'bch') {
        tx.toAddress = walletProvider.useLegacyAddress.useLegacyAddress()
    ...
    // TODO: Call Back Issue
    var isToken = walletProvider.verifyToken(wallet, tx)
      .then(result => {
        return result;
      })
      .catch(() => {
    ...
    if (wallet.coin == BaseCoin) {
      tx.amountStr = this.formatAmountStr(wallet.coin, tx.amount);
      tx.alternativeAmountStr = this.formatAlternativeStr(wallet.coin, tx.amount);

      tx.feeStr = tx.fee
        ? this.formatAmountStr(wallet.coin, tx.fee)
        : tx.fees
          ? this.formatAmountStr(wallet.coin, tx.fees)
          : 'N/A';
      ...
      var token = shareService.getTokenInfo(wallet.coin)
      tx.amountStr = this.formatAmountStr(wallet.coin, tx.amount * token.factor);
      tx.alternativeAmountStr = this.formatAlternativeStr(wallet.coin, tx.amount * token.factor);
      tx.feeStr = tx.fee * token.factor
        ? this.formatAmountStr(wallet.coin, tx.fee * token.factor)
        : tx.fees
          ? this.formatAmountStr(wallet.coin, tx.fees * token.factor)
          : 'N/A';
      if (tx.amountStr) {
        tx.amountValueStr = tx.amountStr.split(' ')[0];
        tx.amountUnitStr = tx.amountStr.split(' ')[1];
      ...
      tx.amountStr = this.formatAmountStr(BaseCoin, tx.amount);
      tx.alternativeAmountStr = this.formatAlternativeStr(BaseCoin, tx.amount);

      tx.feeStr = tx.fee
        ? this.formatAmountStr(BaseCoin, tx.fee)
        : tx.fees
          ? this.formatAmountStr(BaseCoin, tx.fees)
          : 'N/A';
    ...
    if (tx.addressTo && wallet.coin == 'bch') {
      tx.addressTo = walletProvider.useLegacyAddress()
  ...
  public normlizeOutputAmount(coin, tx, shareService: ShareService) {
    var tokenSignatureOutput = tx.outputs.filter(output => {
      return !!output.payload;
    })[0];

    var amountOutput = tx.outputs.filter(output => {
      return !output.payload
    })[0];

    if (tokenSignatureOutput) {
      var tokenInfo = shareService.getTokenInfo(coin, tokenSignatureOutput.payload);
      if (tokenInfo) {
        amountOutput.amount *= tokenInfo.factor;
        if (tx.action == 'received')
          tx.amount = amountOutput.amount;
      }
    }
  }
}
