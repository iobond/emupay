...
export class TxDetailsPage {
  ...
  ionViewDidLoad() {
    ...
    this.blockexplorerUrl = this.getExplorer(this.wallet.coin);
  ...
  private updateTx(opts?): void {
    ...
    this.walletProvider
      ...
      .then(tx => {
        ...
        // Get Memo From OP_RETURN
        for (var i = 0; i < this.btx.outputs.length; i++) {
          // if (this.btx.outputs[i].amount) {
          if (this.btx.outputs[i].amount == 0) {
            this.txMemo = this.hexDecode(this.btx.outputs[i].payload);
          } else {
            this.btx.addressTo = this.btx.outputs[i].address;
          }
          // }
        }
  ...
  private hexDecode(str) {
    if (str) {
      var j;
      var hexes = str.match(/.{1,4}/g) || [];
      var back = "";
      for (j = 0; j < hexes.length; j++) {
        var hex = hexes[j].match(/.{1,2}/g) || [];
        if (hex[0] == "00") {
          back += String.fromCharCode(parseInt(hexes[j].substr(2, 2), 16));
        } else {
  ...
  public viewOnBlockchain(): void {
    ...
    let optIn = true;
    let title = null;
    let message = this.translate.instant('View Transaction on Explorer');
    let okText = this.translate.instant('Open Explorer');
    let cancelText = this.translate.instant('Go Back');
    this.externalLinkProvider.open(
      url,
      optIn,
  ...
  private getExplorer(coin) {
    let defaults = this.configProvider.getDefaults();
    switch (coin) {
      case 'bch': {
        return defaults.blockExplorerUrl.bch;
      }
      case 'btc': {
        return defaults.blockExplorerUrl.btc;
      }
      case 'aib': {
        return defaults.blockExplorerUrl.aib;
      }
      default:
        return defaults.blockExplorerUrl.aib;
    }
