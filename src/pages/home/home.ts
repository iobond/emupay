...
export class HomePage {
  ...
  public showReorderAib: boolean;
  public showReorderToken: boolean;
  ...
  constructor(
    ...
    this.showReorderAib = false;
    this.showReorderToken = false;
  ...
  private setWallets = _.debounce(
    () => {
      ...
      // AIB Upgrade: Critical for wallet showing
      this.walletsAib = _.filter(this.wallets, (x: any) => {
        return x.coin.toUpperCase() == 'AIB'; /* only checking wallet.coin for wallet types */
        /* return x.credentials.coin.toUpperCase() == 'aib';*/
      });

      var tokenWalletTypes = _.filter(this.shareService.walletTypes, (x: any) => {
        return x != BaseCoin.toUpperCase();
      });
      this.walletsToken = _.filter(this.wallets, (x: any) => {
        return _.includes(tokenWalletTypes, x.coin.toUpperCase());
      });
  ...
  private updateAllWallets(): void {
    ...
    let pr = wallet => {
      return new Promise(resolve => {
        this.walletProvider
          .getStatus(wallet, { shareService: this.shareService })
          .then(status => {
            wallet.status = status;
            wallet.error = null;
  ...
  public goToWalletDetails(wallet): void {
    if (this.showReorderBtc || this.showReorderBch || this.showReorderAib || this.showReorderToken) return;
    this.events.unsubscribe('finishIncomingDataMenuEvent');
    this.events.unsubscribe('bwsEvent');
  ...
  public reorderAib(): void {
    this.showReorderAib = !this.showReorderAib;
  }

  public reorderToken(): void {
    this.showReorderToken = !this.showReorderToken;
  }
  ...
  public reorderWalletsAib(indexes): void {
    let element = this.walletsAib[indexes.from];
    this.walletsAib.splice(indexes.from, 1);
    this.walletsAib.splice(indexes.to, 0, element);
    _.each(this.walletsAib, (wallet, index: number) => {
      this.profileProvider.setWalletOrder(wallet.id, index);
    });
  }

  public reorderWalletsToken(indexes): void {
    let element = this.walletsToken[indexes.from];
    this.walletsToken.splice(indexes.from, 1);
    this.walletsToken.splice(indexes.to, 0, element);
    _.each(this.walletsToken, (wallet, index: number) => {
      this.profileProvider.setWalletOrder(wallet.id, index);
    });
  }
