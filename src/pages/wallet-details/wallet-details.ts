...
export class WalletDetailsPage extends WalletTabsChild {
  ...
  private groupHistory(history) {
    return history.reduce((groups, tx, txInd) => {
      if (this.wallet.coin == BaseCoin || this.wallet.coin == tx.amountUnitStr) {
        this.isFirstInGroup(txInd) ? groups.push([tx]) : groups[groups.length - 1].push(tx);
      }
      return groups;
    }, []);
  }
  ...
  private updateStatus(force?: boolean) {
    ...
    this.walletProvider
      .getStatus(this.wallet, { force: !!force, shareService: this.shareService })
      .then(status => {
