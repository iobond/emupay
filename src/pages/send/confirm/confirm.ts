...
export class ConfirmPage extends WalletTabsChild {
  ...
  ionViewWillEnter() {
    ...
    let _bitcore = this.getBWC(this.navParams.data.coin);
    let networkName;
    try {
      networkName = new _bitcore.Address(this.navParams.data.toAddress).network.name;
    } catch (e) {
      ...
      this.getSendMaxInfo(_.clone(tx), wallet)
        .then(sendMaxInfo => {
          if (sendMaxInfo) {
            ...
            if (!!tokenInfo && tokenInfo.factor > 0) {
              sendMaxInfo.amount *= tokenInfo.factor;
            }
  ...
  private getTxp(tx, wallet, dryRun: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (tokenInfo && tokenInfo.factor > 0) {
        txAmount = Math.round(tx.amount / tokenInfo.factor);
        txid = tokenInfo.txid;
      }
      ...
      // txp.nullData = tx.nullData;
      txp.nullData = this.hexEncode(tx.description);
      if (tx.sendMaxInfo) {
  ...
  private confirmTx(_, txp, wallet) {
    return new Promise(resolve => {
      ...
      this.txFormatProvider.formatToUSD(wallet.coin, txp.amount).then(val => {
        ...
        let unit = wallet.coin.toUpperCase();
