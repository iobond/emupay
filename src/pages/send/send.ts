...
export class SendPage extends WalletTabsChild {
  ...
  public walletAibList: FlatWallet[];
  public walletTokenList: FlatWallet[];
  public contactsList = [];
  public filteredContactsList = [];
  public filteredWallets = [];
  public hasBtcWallets: boolean;
  public hasBchWallets: boolean;
  public hasAibWallets: boolean;
  public hasTokenWallets: boolean;
  ...
  ionViewWillEnter() {
    this.walletsBtc = this.profileProvider.getWallets({ coin: Coin.BTC });
    this.walletsBch = this.profileProvider.getWallets({ coin: Coin.BCH });
    this.walletsAib = this.profileProvider.getWallets({ coin: Coin.AIB });
    /* Token Upgrade */
    this.shareService.walletTypes.forEach(walletType =>{
      this.walletsToken.push(this.profileProvider.getWallets({ coin: walletType }));
    });

    this.hasBtcWallets = !_.isEmpty(this.walletsBtc);
    this.hasBchWallets = !_.isEmpty(this.walletsBch);
    this.hasAibWallets = !_.isEmpty(this.walletsAib);
    this.hasAibWallets = !_.isEmpty(this.walletsToken);

    this.walletBchList = this.getBchWalletsList();
    this.walletBtcList = this.getBtcWalletsList();
    this.walletAibList = this.getAibWalletsList();
    /* Token Upgrade */
    this.walletTokenList = this.getTokenWalletsList();
  ...
  private getAibWalletsList(): FlatWallet[] {
    return this.hasBtcWallets ? this.getRelevantWallets(this.walletsAib) : [];
  }

  private getTokenWalletsList(): FlatWallet[] {
    return this.hasBtcWallets ? this.getRelevantWallets(this.walletsToken) : [];
  }
  ...
  private checkCoinAndNetwork(data, walletTypes,isPayPro?): boolean {
    let isValid;
    if (isPayPro) {
        ...
        if (parsedData && parsedData.type == 'PayPro') {
          ...
          this.incomingDataProvider
            ...
            .then(payProDetails => {
              ...
              const isValid = this.checkCoinAndNetwork(payProDetails, this.shareService.walletTypes,true);
          ...
          const isValid = this.checkCoinAndNetwork(this.search,this.shareService.walletTypes,false);
          if (isValid) this.redir();
        } else {
          this.invalidAddress = true;
        }
  ...
  public searchWallets(): void {
    if (this.hasBchWallets && this.wallet.coin === Coin.BCH) {
      this.filteredWallets = this.walletBchList.filter(wallet => {
        return _.includes(wallet.name.toLowerCase(), this.search.toLowerCase());
      });
    }
    if (this.hasBtcWallets && this.wallet.coin === Coin.BTC) {
      this.filteredWallets = this.walletBtcList.filter(wallet => {
        return _.includes(wallet.name.toLowerCase(), this.search.toLowerCase());
      });
    }
    if (this.hasBtcWallets && this.wallet.coin === Coin.AIB) {
      this.filteredWallets = this.walletBtcList.filter(wallet => {
        return _.includes(wallet.name.toLowerCase(), this.search.toLowerCase());
      });
    }
    if (this.hasBtcWallets && this.shareService.isValidToken(this.wallet.coin)) {
      this.filteredWallets = this.walletBtcList.filter(wallet => {
        return _.includes(wallet.name.toLowerCase(), this.search.toLowerCase());
      });
    }
