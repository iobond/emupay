...
export class AmountPage extends WalletTabsChild {
  ...
  private setAvailableUnits(): void {
    ...
    if (this.shareService.isValidToken(this.wallet.coin)) {
      this.availableUnits.push({
        name: this.wallet.coin,
        id: this.wallet.coin,
        shortName: this.wallet.coin,
      });
    }

    if (parentWalletCoin === Coin.AIB.toUpperCase() || !parentWalletCoin) {
      this.availableUnits.push({
        name: 'AIB',
        id: Coin.AIB,
        shortName: Coin.AIB.toUpperCase(),
      });
    }
    ...
    if (parentWalletCoin === 'btc' || !parentWalletCoin) {
      this.availableUnits.push({
        name: 'Bitcoin',
        id: Coin.BTC,
        shortName: Coin.BTC.toUpperCase()
      });
    }
    ...
    if (parentWalletCoin === 'bch' || !parentWalletCoin) {
      this.availableUnits.push({
        name: 'Bitcoin Cash',
        id: Coin.BCH,
        shortName: Coin.BCH.toUpperCase()
      });
    }
  ...
  public sendMax(): void {
    ...
    var availableBalanceSat = this.wallet.status.availableBalanceSat;
    if (!this.wallet.status.availableBalanceSat) {
      var tokenInfo = this.shareService.getTokenInfo(this.wallet.coin);
      var tokens = this.wallet.status.balance.tokens;
      if (!!tokenInfo && !!tokens && tokens.length>0) {
          availableBalanceSat = tokens[0].availableAmount;
      }
    }
